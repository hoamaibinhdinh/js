$(function(){
	...
	var Fireworks = function(){
		...
		/* Initialize
		/*=============================================================================*/
		$('body').append('<div class="fireworks-container"><div class="stage-container"><div class="canvas-container"><canvas></canvas></div></div></div>');
		self.init = function(){	
			self.canvas = $('body').find('canvas')[0];				
		...
		self.getRequestParam = function(name){
		   if(name=(new RegExp('[?&]'+encodeURIComponent(name)+'=([^&]*)')).exec(location.search))
			  return decodeURIComponent(name[1]);
		};
		
		
		self.totalCPUCores = navigator.hardwareConcurrency || 2;
		self.customFireWorks = self.getRequestParam('fws');
		self.isDesktop = $(window).innerWidth() > 1024;
		self.totalFireworks = 6;
		self.maxFireWorks = typeof self.customFireWorks === 'undefined' ? self.totalFireworks : self.customFireWorks;
		
		self.runOnLoad = function(totalFW, count){
			count = typeof count === 'undefined' ? 0 : count + 1;
			var cRand = rand(200, 800);
			var isDesktop = self.canvas.width > 1024;
			var midW = self.canvas.width/2;
			var midH = self.canvas.height/2;
			
			setTimeout(function(){
				if (self.fireworks.length <= self.totalCPUCores && totalFW < self.maxFireWorks) {
					var yDes = rand(100, 400);
					self.createFireworks(0, self.canvas.height, 200, yDes > midH ? 100 : yDes);	
					totalFW++;
				}
				setTimeout(function(){
					if (self.fireworks.length <= self.totalCPUCores && totalFW < self.maxFireWorks) {
						var yDes = rand(100, 400);
						var xLeft = rand(self.canvas.width - 250, self.canvas.width - 150);
						self.createFireworks(self.canvas.width, self.canvas.height, (xLeft + 200) > self.canvas.width ? self.canvas.width - 150 : xLeft, yDes > midH ? 100 : yDes);
						totalFW++;
					}
					self.currentHue = rand(self.hueMin, self.hueMax);
					if (totalFW < self.maxFireWorks) {
						self.runOnLoad(totalFW, count);
					} else {
						$('body > canvas').addClass('fw-hide');
						setTimeout(function(){
							$('body').removeClass('fw-show');
						}, 2000);
					}
				}, cRand);
			}, isDesktop ? 0 : 2000);
		...
		self.bindEvents = function(){
			...
			$(document).ready(function(){			
				$('body').addClass('fw-show');
				
				console.log('Cores', self.totalCPUCores);
				self.runOnLoad(0);
			});				
		}
